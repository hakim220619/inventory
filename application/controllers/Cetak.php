...
class Cetak extends CI_Controller
{
    ...
    public function history_penjualan_ex($a, $b, $id)
    {
        $data['user'] = $this->db->get_where('user', ['id' => $this->session->userdata('id')])->row_array();
        $data['title'] = "Cetak History Penjualan";
        $data['p_umum'] = $this->db->get_where('pengaturan_umum', ['id' => 1])->row_array();
        ...
        if ($id == 0) {
            ...
            $data['history'] = $this->db->query($historya)->result_array();
            $data['c'] = "Semua Cabang";
        } else {
            ...
            $data['c'] = $c['nama_cabang'];
        }
        $this->load->view('templates/supadmin/cetak_header', $data);
        $this->load->view('supadmin/cetak/c_history_penjualan_ex', $data);
        $this->load->view('templates/supadmin/cetak_footer', $data);
    }
    ...
    public function laporan_penjualan_hari_ex($a, $b, $id)
    {
        $data['idid'] = $id;
        $data['dats'] = "SELECT DISTINCT tanggal_ind FROM riwayat_penjualan WHERE tanggal_ind BETWEEN '$a' AND '$b' AND id_cabang='$id'";
        ...
        $data['data_barang'] = $this->db->get('barang')->result_array();
        $this->load->view('templates/supadmin/cetak_header', $data);
        $this->load->view('supadmin/cetak/c_laporan_penjualan_ex', $data);
        $this->load->view('templates/supadmin/cetak_footer', $data);
    }
    ...
    public function laporan_pengeluaran_ex($dari, $ke, $id)
    {
        if ($id == 0) {
            ...
            $data['pengeluaran'] = $this->db->query($query)->result_array();
            $data['header'] = $cabang_a['nama_cabang'];
            $data['cabang']= $cabang_a;
        }
        $data['user'] = $this->db->get_where('user', ['id' => $this->session->userdata('id')])->row_array();
        $data['title'] = "Cetak Laporan Penjualan";
        $data['p_umum'] = $this->db->get_where('pengaturan_umum', ['id' => 1])->row_array();
        ...
        $this->load->view('templates/supadmin/cetak_header', $data);
        $this->load->view('supadmin/cetak/c_laporan_pengeluaran_ex', $data);
        $this->load->view('templates/supadmin/cetak_footer', $data);
    }
    ...
    public function laporan_pengeluaran_hari_ex($dari, $ke, $id)
    {
        $data['title'] = "Cetak Laporan pengeluaran Hari";
        $data['p_umum'] = $this->db->get_where('pengaturan_umum', ['id' => 1])->row_array();
        $data['dats'] = "SELECT DISTINCT tanggal_ind FROM riwayat_pengeluaran WHERE status_bukti != '0' AND tanggal_ind BETWEEN '$dari' AND '$ke' AND id_cabang = '$id'";

        $data['user'] = $this->db->get_where('user', ['id' => $this->session->userdata('id')])->row_array();
        $this->load->view('templates/supadmin/cetak_header', $data);
        $this->load->view('supadmin/cetak/c_laporan_pengeluaran_h_ex', $data);
        $this->load->view('templates/supadmin/cetak_footer', $data);
    }
